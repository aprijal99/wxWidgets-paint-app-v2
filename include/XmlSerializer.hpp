...
struct XmlSerializer
{
  ...
  wxXmlDocument SerializePaths(const std::vector<Path>& paths)
  {
    ...
    for (const Path& path: paths)
    {
      ...
      for (const wxPoint2DDouble& point: path.points)
      {
  ...
  std::vector<Path> DeserializePaths(const wxXmlDocument& doc)
  {
    ...
    {
      if (node->GetName() != "Object") continue;

      if (node->GetAttribute("type") == "Path")
      {
        Path squiggle;
        squiggle.color = wxColour(node->GetAttribute("color"));
        ...
        {
          if (pointNode->GetName() != "Point") continue;

          squiggle.points.push_back(wxPoint2DDouble(
  ...
  {
    wxFileOutputStream outStream(zipFile);
